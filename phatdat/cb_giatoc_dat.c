...
#define INT 11
float t,tp,dt,w,wp,pitch_p, pitch;
int mpu;
...
void InitValue( void )
{
	...
	pitch_p = 	atan2(Ay, sqrt(pow(Ax,2)+pow(Az,2)))*180/M_PI;
	wp=(float)read_sensor(gyro_y)/65.5;
	tp = 0;
}
...
void read_mpu(void)
{
	...
	dt=t-tp;
	// doc sensor
	w=(float)read_sensor(gyro_y)/65.5;
	pitch = 0.5*(wp+w)*dt/1000 + pitch_p;
